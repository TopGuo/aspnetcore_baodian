...
namespace TB.AspNetCore.Application.Services
{
    public class PermissionService : BaseService, IPermissionService
    {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public ResponsResult SaveRoles(RoleModel model)
        {
            ResponsResult result = new ResponsResult();
            var list = base.Query<SystemRoles>().ToList();
        ...
        public void RegistRole()
        {
            ...
            //获取最新的角色权限关系
            rolePermmissionLists.Where(t => !actionIds.Contains(t.ActionId)).ToList().ForEach((Action<SystemRolePermission>)(x =>
            {
                base.Delete((object)x);
            }));
